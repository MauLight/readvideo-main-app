...
/** The caller's keys travel per request — the server holds none of its own. */
...
function statusFor(err: unknown): number {
  ...
  if (err instanceof TranscriptError) return 422;
  return 500;
}
 ...
 * first frame.
 *
 * Generic over the runner, so a second one can be mounted the same way without
 * touching this file.
 */
