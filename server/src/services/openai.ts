...
/**
 * Clients are per-key since credentials belong to the caller, but building one
 * per request is wasteful — the SDK holds a connection pool. Cached by key so
 * repeated calls from the same caller reuse it.
 */
...
Delimit math with dollar signs — $...$ inline and $$...$$ for a formula on its own line. Do not use \\( \\) or \\[ \\], which many Markdown renderers do not parse.
